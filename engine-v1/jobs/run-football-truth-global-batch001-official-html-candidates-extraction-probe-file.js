...
const inputPath = path.join(root, "data", "football-truth", "_diagnostics", `football-truth-global-batch001-strict-official-host-pattern-probe-${today}`, `football-truth-global-batch001-strict-official-host-pattern-probe-${today}.json`);
const inputRowsPath = path.join(root, "data", "football-truth", "_diagnostics", `football-truth-global-batch001-strict-official-host-pattern-probe-${today}`, `football-truth-global-batch001-strict-official-host-pattern-probe-rows-${today}.jsonl`);
const inputVerificationPath = path.join(root, "data", "football-truth", "_diagnostics", `football-truth-global-batch001-strict-official-host-pattern-probe-verification-${today}`, `football-truth-global-batch001-strict-official-host-pattern-probe-verification-${today}.json`);

const outDir = path.join(root, "data", "football-truth", "_diagnostics", `football-truth-global-batch001-official-html-candidates-extraction-probe-${today}`);
const outPath = path.join(outDir, `football-truth-global-batch001-official-html-candidates-extraction-probe-${today}.json`);
const rowsPath = path.join(outDir, `football-truth-global-batch001-official-html-candidates-extraction-probe-rows-${today}.jsonl`);
...
function headerScore(cells) {
  ...
  for (const rx of [
    ...
    /\b(team|club|name|equipo|ομαδα)\b/,
    /\b(mp|pj|p|played|games|matches|pld|j|αγωνες|αγ)\b/,
    /\b(w|wins|won|pg|v|νικες|ν)\b/,
    /\b(d|draws|draw|pe|ισοπαλιες|ι)\b/,
    /\b(l|losses|lost|pp|ηττες|η)\b/,
    /\b(gf|ga|gd|dg|diff|goals|for|against)\b/,
    /\b(pts|points|puntos|βαθμοι|β)\b/
...
function columnMap(header, width) {
  ...
  for (let i = 0; i < hs.length; i++) {
    ...
    if (map.played == null && /^(mp|pj|p|played|games|matches|pld|j|αγωνες|αγ)$/.test(h)) map.played = i;
    if (map.wins == null && /^(w|wins|won|pg|v|νικες|ν)$/.test(h)) map.wins = i;
    if (map.draws == null && /^(d|draws|draw|pe|ισοπαλιες|ι)$/.test(h)) map.draws = i;
    if (map.losses == null && /^(l|losses|lost|pp|ηττες|η)$/.test(h)) map.losses = i;
    if (map.goalsFor == null && /^(gf|goals for|for|f|υπερ|γυ)$/.test(h)) map.goalsFor = i;
    if (map.goalsAgainst == null && /^(ga|gc|goals against|against|a|κατα|γκ)$/.test(h)) map.goalsAgainst = i;
    if (map.goalDifference == null && /^(gd|dg|diff|difference|διαφ)$/.test(h)) map.goalDifference = i;
    if (map.points == null && /^(pts|points|puntos|pt|βαθμοι|β)$/.test(h)) map.points = i;
  }
...
function extractStandingsRows(table) {
  ...
  let headerRowIndex = rows.findIndex(row => headerScore(row) >= 3);
  ...
  for (const cells of rows.slice(headerRowIndex + 1)) {
    ...
    if (!teamName || /^\d+$/.test(teamName) || teamName.includes("{{")) {
      const idx = cells.findIndex((cell, i) => i !== map.position && parseIntLoose(cell) == null && stripHtml(cell).length >= 2 && !stripHtml(cell).includes("{{"));
    ...
    if ((played == null || wins == null || draws == null || losses == null || points == null) && numeric.length >= 5) {
      const start = position != null && numeric[0] === position ? 1 : 0;
      played = played ?? numeric[start] ?? null;
      wins = wins ?? numeric[start + 1] ?? null;
      draws = draws ?? numeric[start + 2] ?? null;
      losses = losses ?? numeric[start + 3] ?? null;
      if (numeric.length - start >= 8) {
        goalsFor = goalsFor ?? numeric[start + 4] ?? null;
        goalsAgainst = goalsAgainst ?? numeric[start + 5] ?? null;
        goalDifference = goalDifference ?? numeric[start + 6] ?? null;
        points = points ?? numeric[start + 7] ?? null;
      } else {
        points = points ?? numeric[numeric.length - 1] ?? null;
      }
    }

    if (!teamName || teamName.length < 2 || teamName.includes("{{")) continue;
...
function scoreTable(table, extraction) {
  ...
  const arithmetic = extraction.standingsRows.filter(row => row.arithmeticPassed).length;
  const nonzero = extraction.standingsRows.filter(row => Number(row.played || 0) > 0).length;
  return headerMaxScore * 30 + extracted * 15 + arithmetic * 22 + nonzero * 12 + Math.min(rowCount, 40) + maxCellCount;
}
...
async function fetchWithTimeout(url, timeoutMs = 12000) {
  ...
  try {
    const response = await fetch(url, {
      ...
      headers: {
        "user-agent": "Mozilla/5.0 (compatible; AI-MatchLab-FootballTruth/1.0; +official-html-16-extraction)",
...
const targets = inputRows.filter(row => row.selectedLane === "official_html_table_surface_candidate");
if (targets.length !== 16) blocks.push("target_rows_not_16");
...
if (allowFetch && blocks.length === 0) {
  ...
  for (const target of targets) {
    i += 1;
    const url = target.selectedFinalUrl || target.selectedUrl;
    ...
    const arithmeticPassedRowCount = standingsRows.filter(row => row.arithmeticPassed).length;
    const arithmeticFailedRowCount = standingsRows.filter(row => row.arithmeticPassed === false).length;
    ...
    rows.push({
      ...
      selectedLane: target.selectedLane,
      sourceSelectedUrl: target.selectedUrl,
      sourceSelectedFinalUrl: target.selectedFinalUrl,
      ...
      selectedTableScore: selected?.score ?? null,
      selectedTableRowCount: selected?.table?.rows?.length ?? 0,
      selectedTableMaxCellCount: selected ? Math.max(0, ...selected.table.rows.map(row => row.length)) : 0,
...
const report = {
  ...
  runner: "global_batch001_official_html_candidates_extraction_probe",
  contractVersion: 1,
  output: rel(outPath),
  ...
  guardrails: {
    ...
    controlledHtmlCandidateExtractionFetchExecutedNowCount: fetchCount,
  ...
  summary: {
    ...
    acceptedNowCount: 0,
    nextRecommendedLane: "proof-shape rows require season/lifecycle review and explicit approval before any candidate write"
  },
  rows,
  blocks
};
