...
const diagnosticPath = path.join(root, "data", "football-truth", "_diagnostics", `football-truth-global-batch001-strict-official-host-pattern-probe-${today}`, `football-truth-global-batch001-strict-official-host-pattern-probe-${today}.json`);
const rowsPath = path.join(root, "data", "football-truth", "_diagnostics", `football-truth-global-batch001-strict-official-host-pattern-probe-${today}`, `football-truth-global-batch001-strict-official-host-pattern-probe-rows-${today}.jsonl`);
const verificationDir = path.join(root, "data", "football-truth", "_diagnostics", `football-truth-global-batch001-strict-official-host-pattern-probe-verification-${today}`);
const verificationPath = path.join(verificationDir, `football-truth-global-batch001-strict-official-host-pattern-probe-verification-${today}.json`);
...
if (diagnostic.runner !== "global_batch001_strict_official_host_pattern_probe") blocks.push("runner_mismatch");
if (diagnostic.summary?.targetCount !== 80) blocks.push("target_count_not_80");
if (rows.length !== 80) blocks.push("rows_not_80");
if ((diagnostic.summary?.attemptedFetchCount || 0) < 100) blocks.push("too_few_fetches_for_pattern_probe");
if (!diagnostic.summary?.selectedLaneCounts) blocks.push("missing_lane_counts");

for (const row of rows) {
  if (!row.slug) blocks.push("row_missing_slug");
  if (!row.selectedLane) blocks.push(`missing_lane_${row.slug}`);
...
if (guardrails.fetchExecutedNowCount !== diagnostic.summary?.attemptedFetchCount) blocks.push("fetch_count_mismatch");
...
const verification = {
  ...
  runner: "verify_global_batch001_strict_official_host_pattern_probe",
  contractVersion: 1,
  ...
  verified: {
    ...
    selectedLaneCounts: diagnostic.summary.selectedLaneCounts,
    officialHtmlTableSurfaceCandidateSlugs: diagnostic.summary.officialHtmlTableSurfaceCandidateSlugs,
    officialRenderedOrApiSurfaceCandidateSlugs: diagnostic.summary.officialRenderedOrApiSurfaceCandidateSlugs,
    officialFixtureOrScheduleOnlySlugs: diagnostic.summary.officialFixtureOrScheduleOnlySlugs,
    officialSurfaceReviewRequiredSlugs: diagnostic.summary.officialSurfaceReviewRequiredSlugs,
    officialRouteNotFoundOrBlockedSlugs: diagnostic.summary.officialRouteNotFoundOrBlockedSlugs,
    guardrailsHeld: blocks.length === 0
  },
  conclusion: "Strict official-host pattern probe is verified. It avoids contaminated local-evidence URLs and probes only official host allowlists with bounded route patterns. No route/family/canonical/lifecycle/production/truth claim was executed.",
  blocks
};
