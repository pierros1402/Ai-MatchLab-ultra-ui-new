...
const outputDir = path.join("data","football-truth","_diagnostics","whole-map-high-volume-unresolved-bulk-followup-wave-next-execution-2026-06-16");
const responseDir = path.join(outputDir,"responses");
const outputPath = path.join(outputDir,"whole-map-high-volume-unresolved-bulk-followup-wave-next-execution-2026-06-16.json");
...
if(preflightBlockedCount || !allowExecute || !allowFetch){
  ...
  const output = {
    output:outputPath,
    job:"run-football-truth-whole-map-high-volume-unresolved-bulk-followup-wave-next-execution-file",
