...
const seedStartIndex = Number(process.argv.find(a=>a.startsWith("--seed-start-index="))?.split("=")[1] ?? 0);
const waveTag = `whole-map-high-volume-official-domain-seed-wave-start-${seedStartIndex}-2026-06-16`;
...
const outputDir = path.join("data","football-truth","_diagnostics",waveTag);
const responseDir = path.join(outputDir,"responses");
const outputPath = path.join(outputDir,`${waveTag}.json`);
...
function buildSeedRows(pack){
  ...
  return uniqueRows.slice(seedStartIndex,seedStartIndex+maxUrls);
}
