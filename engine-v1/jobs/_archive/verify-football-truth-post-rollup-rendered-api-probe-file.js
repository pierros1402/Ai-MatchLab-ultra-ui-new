...
const reportPath = path.join(root, "data", "football-truth", "_diagnostics", `football-truth-post-rollup-rendered-api-probe-${today}`, `football-truth-post-rollup-rendered-api-probe-${today}.json`);
const rowsPath = path.join(root, "data", "football-truth", "_diagnostics", `football-truth-post-rollup-rendered-api-probe-${today}`, `football-truth-post-rollup-rendered-api-probe-rows-${today}.jsonl`);
const verificationDir = path.join(root, "data", "football-truth", "_diagnostics", `football-truth-post-rollup-rendered-api-probe-verification-${today}`);
const verificationPath = path.join(verificationDir, `football-truth-post-rollup-rendered-api-probe-verification-${today}.json`);
...
if (report.runner !== "football_truth_post_rollup_rendered_api_probe") blocks.push("runner_mismatch");
if (report.summary?.targetCount !== 4) blocks.push("target_count_not_4");
if (rows.length !== 4) blocks.push("rows_not_4");
if (JSON.stringify(sorted(rows.map(row => row.slug))) !== JSON.stringify(["eng.4","eng.5","gha.2","irl.2"])) blocks.push("slug_set_mismatch");
if ((report.summary?.attemptedBrowserRenderCount || 0) <= 0) blocks.push("attempted_browser_render_not_positive");
if (report.summary?.attemptedBrowserRenderCount !== report.guardrails?.browserRenderExecutedNowCount) blocks.push("browser_render_count_mismatch");

for (const row of rows) {
  if (!row.slug) blocks.push("row_missing_slug");
  if (!row.renderedFinalLane) blocks.push(`missing_rendered_final_lane_${row.slug}`);
...
const verification = {
  ...
  runner: "verify_football_truth_post_rollup_rendered_api_probe",
  contractVersion: 1,
  ...
  conclusion: "Post-rollup rendered/API probe is verified. It browser-renders the rendered/API queue and classifies extracted output without search/fetch/candidate/canonical/lifecycle/production/truth write.",
  blocks
};
