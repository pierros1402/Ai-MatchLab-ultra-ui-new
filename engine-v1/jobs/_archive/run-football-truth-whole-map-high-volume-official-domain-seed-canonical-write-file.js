...
const planPath = path.join("data","football-truth","_diagnostics","whole-map-high-volume-official-domain-seed-accepted-canonical-plan-2026-06-16","whole-map-high-volume-official-domain-seed-accepted-canonical-plan-2026-06-16.json");
const canonicalPath = path.join("data","football-truth","_state","canonical-standings-candidates","official-domain-seed-austria-standings-candidates-2026-06-16.json");
const outputPath = path.join("data","football-truth","_diagnostics","whole-map-high-volume-official-domain-seed-canonical-write-2026-06-16","whole-map-high-volume-official-domain-seed-canonical-write-2026-06-16.json");
const continuationNotePath = path.join("data","football-truth","_diagnostics","continuation-note-2026-06-16-after-austria-canonical-write","continuation-note-2026-06-16-after-austria-canonical-write.txt");
...
check(checks,"sourcePlanShaMatchesExpected",plan.summary?.plannedCanonicalCandidateSha256==="0f2b15232ccaa38061e1ac0e67743897a0ba8c9f8b87a62a9cd64a3606431b9e",{actual:plan.summary?.plannedCanonicalCandidateSha256});
check(checks,"competitionCountTwo",canonical.summary.competitionCount===2,{actual:canonical.summary.competitionCount});
check(checks,"candidateRowsTwentyEight",canonical.summary.candidateRowCount===28,{actual:canonical.summary.candidateRowCount});
check(checks,"autOneRowsTwelve",canonical.summary.rowsByCompetition["aut.1"]===12,{actual:canonical.summary.rowsByCompetition["aut.1"]});
check(checks,"autTwoRowsSixteen",canonical.summary.rowsByCompetition["aut.2"]===16,{actual:canonical.summary.rowsByCompetition["aut.2"]});
...
if(blockedCheckCount) {
  writeJson(outputPath,{output:outputPath,job:"run-football-truth-whole-map-high-volume-official-domain-seed-canonical-write-file",status:"blocked",checks,summary:{status:"blocked",blockedCheckCount,passedCheckCount,canonicalWriteExecutedNowCount:0,fetchExecutedNowCount:0,searchExecutedNowCount:0,broadSearchExecutedNowCount:0,productionWriteExecutedNowCount:0,truthAssertionExecutedNowCount:0}});
...
const output = {
  output: outputPath,
  job: "run-football-truth-whole-map-high-volume-official-domain-seed-canonical-write-file",
  generatedAtUtc: new Date().toISOString(),
  sourcePlanPath: planPath,
  sourcePlanSha256: sha256Text(planText),
  canonicalPath,
...
- Wrote canonical candidate file for Austria official domain seed accepted rows:
  ${canonicalPath}
- aut.1: 12 rows
- aut.2: 16 rows
- total: 28 rows
...
- aut.1, aut.2: official domain seed canonical candidate file, 28 rows
- Total reliable intended canonical candidate rows: 250 across 15 leagues.
  Note: one diagnostic recursive counter overcounted some files as 229 before Austria; use the known intended file counts above as the reliable checkpoint.
...
- Direct official-domain seed wave: 700 bounded fetches over 16 competitions / 7 countries.
  Results: 254 fetched 2xx; best fetched 2xx for all 16; accepted exact expected HTML table rows for aut.1/aut.2 only; 14 still no rows.
- Official domain seed accepted canonical plan passed for aut.1/aut.2, sha 0f2b15232ccaa38061e1ac0e67743897a0ba8c9f8b87a62a9cd64a3606431b9e.
