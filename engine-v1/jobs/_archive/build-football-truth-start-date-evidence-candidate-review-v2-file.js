...
const OUT_DIR = path.join(DATA_ROOT, "_diagnostics", `start-date-evidence-candidate-review-v2-${DATE}`);
...
function collectDateMentionsFromContext(context) {
  const raw = String(context || "").replace(/\s+/g, " ");
  const lower = raw.toLowerCase();
  const mentions = [];

  const add = (date, index, matchedText) => {
    if (!date) return;
    const window = raw.slice(Math.max(0, index - 180), Math.min(raw.length, index + 220)).replace(/\s+/g, " ").trim();
    mentions.push({ date, index, matchedText, context: window, score: scoreDateMention(window, date, index, raw) });
  };
  ...
  for (const m of lower.matchAll(dayMonthYear)) {
    add(normalizeDate(m[3], monthMap.get(m[2].toLowerCase()), m[1]), m.index || 0, m[0]);
  }
  ...
  return mentions.sort((a, b) => b.score - a.score || a.date.localeCompare(b.date));
}

function scoreDateMention(window, date, index, fullText) {
  const w = String(window || "").toLowerCase();
  let score = 0;

  if (/\b(will start|will begin|is set to start|set to start|starts on|start on|begins on|kick.?off on|opening match round.*start|opening round.*start)\b/i.test(w)) score += 10;
  if (/\b(first match|opening match|opening game|season opener|opening fixture|matchday 1|round 1)\b/i.test(w)) score += 6;
  if (/\b(start|starts|begin|begins|kick.?off|commence|commences)\b/i.test(w)) score += 4;
  if (/\b(2026\/27|2026-27|2026\/2027|2026-2027|new campaign|new season)\b/i.test(w)) score += 3;
  if (/\b(premier league|championship|eredivisie|league|season)\b/i.test(w)) score += 2;

  if (/\b(published|updated|last updated|article|news|awards|player of the year|goal of the year|calendar for|events calendar|concerts|solar eclipse|travel|tourism)\b/i.test(w)) score -= 8;
  if (/\b(privacy|cookies|terms|newsletter|subscribe)\b/i.test(w)) score -= 3;

  const datePos = String(fullText || "").indexOf(date);
  if (datePos >= 0 && datePos < 100) score -= 4;
...
function bestStartDateFromContexts(contexts) {
  ...
  for (const context of contexts || []) {
    mentions.push(...collectDateMentionsFromContext(context));
  }
...
for (const c of candidateClassifications) {
  ...
  const bestMention = bestStartDateFromContexts([...new Set(contexts)]);
  ...
  if (selectedContextScore < 8) reasons.push("reject_selected_date_context_not_strict_enough");
  if (selectedDate && c.candidateNextSeasonStartDate && selectedDate !== c.candidateNextSeasonStartDate) reasons.push("corrected_candidate_date_from_page_or_article_date");

  const reviewStatus =
    slugValid &&
    knownOfficial &&
    selectedDate &&
    selectedContextScore >= 8
      ? "accepted_strict_official_start_date_v2"
      : "rejected_or_manual_review_v2";
  ...
  reviewRows.push({
    ...
    selectedEvidenceContext: bestMention?.context || null,
    allEvidenceContexts: [...new Set(contexts)].slice(0, 10)
  });
}

const accepted = reviewRows.filter((r) => r.reviewStatus === "accepted_strict_official_start_date_v2");
const rejected = reviewRows.filter((r) => r.reviewStatus !== "accepted_strict_official_start_date_v2");
...
const summary = {
  status: "passed",
  runner: "start_date_evidence_candidate_review_v2",
  ...
  correctedCandidateDateCount: reviewRows.filter((r) => r.decisionReasons.includes("corrected_candidate_date_from_page_or_article_date")).length,
  hostRankingKnownOfficialNotFirstCount: hostRankingIssues.length,
  recommendedNextLane: accepted.length
    ? "backfill_nextSeasonStartDate_from_v2_accepted_candidates_then_rerun_coverage_ledger"
    : "fix_host_ranking_and_run_source_specific_official_calendar_fetchers"
};

const outPath = path.join(OUT_DIR, `start-date-evidence-candidate-review-v2-${DATE}.json`);
const rowsPath = path.join(OUT_DIR, `start-date-evidence-candidate-review-v2-rows-${DATE}.jsonl`);
const acceptedPath = path.join(OUT_DIR, `accepted-start-date-evidence-candidates-v2-${DATE}.jsonl`);
const rejectedPath = path.join(OUT_DIR, `rejected-start-date-evidence-candidates-v2-${DATE}.jsonl`);
const hostRankingPath = path.join(OUT_DIR, `host-ranking-repair-findings-v2-${DATE}.jsonl`);
