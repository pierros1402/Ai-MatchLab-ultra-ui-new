...
const DEFAULTS = {
  date: "2026-06-14",
  approvalInput: "data/football-truth/_diagnostics/final-explicit-controlled-sportomedia-graphql-query-body-recovery-execution-run-approval-2026-06-14/final-explicit-controlled-sportomedia-graphql-query-body-recovery-execution-run-approval-2026-06-14.json",
  output: "data/football-truth/_diagnostics/controlled-sportomedia-graphql-query-body-recovery-execution-run-2026-06-14/controlled-sportomedia-graphql-query-body-recovery-execution-run-2026-06-14.json",
...
function validateApproval(input) {
  ...
  assertSummary(s, "finalExplicitControlledSportomediaGraphqlQueryBodyRecoveryExecutionRunApprovalCompetitionCount", 2);
  assertSummary(s, "finalExplicitControlledSportomediaGraphqlQueryBodyRecoveryExecutionRunApprovalApprovedCount", 2);
  assertSummary(s, "finalExplicitControlledSportomediaGraphqlQueryBodyRecoveryExecutionRunApprovalBlockedCount", 0);
  ...
  assertSummary(s, "mayRunControlledQueryBodyRecoveryNextCount", 2);
  assertSummary(s, "finalRunWouldAllowControlledQueryBodyRecoveryCount", 2);
  ...
  for (const row of rows) {
    if (row.finalApprovalStatus !== "approved_for_next_step_controlled_sportomedia_graphql_query_body_recovery_execution") {
      throw new Error(row.competitionSlug + ": final approval is not approved.");
    }
    if (row.mayRunControlledQueryBodyRecoveryNext !== true) {
      throw new Error(row.competitionSlug + ": mayRunControlledQueryBodyRecoveryNext must be true.");
    }
    if (row.finalRunWouldAllowControlledQueryBodyRecovery !== true) {
      throw new Error(row.competitionSlug + ": controlled query/body recovery must be allowed for this run.");
    }
    ...
    if (!row.primaryOperationCandidate) throw new Error(row.competitionSlug + ": missing approved primary operation candidate.");
  }
  ...
  for (let i = 0; i < s.length; i += 1) {
    ...
    if (ch === "{") {
      ...
      if (stack.length === 0 && start >= 0) {
        ...
        if (candidate.length >= 20 && /(query|variables|operationName|extensions|persisted|sha256|documentId|operationId|queryId|id)/i.test(candidate)) {
          out.push(candidate);
        }
        start = -1;
      }
...
function findNestedGraphqlBody(value, depth = 0) {
  ...
  for (const key of Object.keys(value)) {
    ...
    if (typeof nested === "string" && /(query|variables|operationName|extensions|persisted|sha256|documentId|operationId|queryId|id)/i.test(nested)) {
...
function buildRequestBodyFromCandidate(candidate) {
  const primary = candidate || {};
  const rawSnippet = String(primary.rawSnippet || "");
  const query = primary.query || null;
  const persistedKey = primary.persistedKey || null;
  const persistedValue = primary.persistedValue || null;

  if (query) {
    return {
      body: {
        query,
        variables: primary.variables && typeof primary.variables === "object" ? primary.variables : {}
      },
      bodyBuildStatus: "built_from_full_query_text",
      requestBodyVariant: "query_text"
    };
  }

  const parsed = parseJsonishObject(rawSnippet);
  const nested = findNestedGraphqlBody(parsed);
  if (nested) {
    const sanitized = sanitizeGraphqlBody(nested);

    if (Object.keys(sanitized).length > 1 || sanitized.query || sanitized.extensions || sanitized.id || sanitized.queryId || sanitized.documentId || sanitized.operationId) {
      return {
        body: sanitized,
        bodyBuildStatus: "built_from_body_like_candidate",
        requestBodyVariant: "body_like_candidate"
      };
    }
  }

  if (persistedValue) {
    const key = String(persistedKey || "").trim();

    if (/sha|hash/i.test(key)) {
      return {
        body: {
          extensions: {
            persistedQuery: {
              version: 1,
              sha256Hash: persistedValue
            }
          },
          variables: {}
        },
        bodyBuildStatus: "built_from_persisted_query_hash_candidate",
        requestBodyVariant: "apollo_persisted_query_hash_candidate"
      };
    }
    ...
    if (key) {
      return {
        ...
        bodyBuildStatus: "built_from_persisted_query_id_candidate",
        requestBodyVariant: "persisted_query_id_candidate"
      };
    }
    ...
    return {
      ...
      bodyBuildStatus: "built_from_generic_persisted_query_id_candidate",
      requestBodyVariant: "generic_persisted_query_id_candidate"
    };
  }

  return {
    body: null,
    bodyBuildStatus: "blocked_unable_to_build_request_body_from_primary_candidate",
...
async function buildExecutionRow(row, args) {
  ...
  const built = buildRequestBodyFromCandidate(primary);
  ...
  const base = {
    ...
    primaryCandidateType: primary.candidateType || null,
    primaryCandidateScore: primary.candidateScore ?? null,
    primaryCandidateSource: primary.source || null,
    primaryCandidateSha256: sha256(JSON.stringify(primary)),
    ...
    requestBodyPreview: built.body ? JSON.stringify(built.body).slice(0, 1000) : null,

    finalRunWouldAllowControlledQueryBodyRecovery: row.finalRunWouldAllowControlledQueryBodyRecovery,
    ...
    queryBodyCandidateResponseIsTruth: false,
    canonicalWrites: 0,
  ...
  if (!built.body) {
    return {
      ...base,
      executionStatus: "blocked_request_body_build_failed",
      ...
      queryBodyRecoveryResponseCandidate: false,
      nextAllowedStep: "repair_query_body_request_body_builder_from_candidate",
  ...
  const queryBodyRecoveryResponseCandidate =
  ...
  const executionStatus =
    fetchResult.fetchStatus !== "fetched_ok"
      ? "query_body_recovery_fetch_error"
      : fetchResult.status < 200 || fetchResult.status >= 300
        ? "query_body_recovery_http_not_ok"
        : queryBodyRecoveryResponseCandidate
          ? "query_body_recovery_candidate_response_received"
          : responseHasErrorsKey
            ? "query_body_recovery_graphql_error_response"
            : "query_body_recovery_response_received_but_not_payload_ready";
  ...
  return {
    ...
    nextAllowedStep:
      queryBodyRecoveryResponseCandidate
        ? "review_query_body_recovery_candidate_response_and_build_no_write_sportomedia_payload_parser"
        : "review_query_body_recovery_response_and_refine_request_body_candidate",
    nextBlockedStep: "classifier_canonical_write_production_write_truth_assertions_blocked"
  };
}
...
async function main() {
  ...
  const output = {
    ...
    date: args.date,
    job: "run-football-truth-controlled-sportomedia-graphql-query-body-recovery-execution-file",
    mode: "controlled_diagnostics_only_sportomedia_graphql_query_body_recovery_execution_fetch_allowed_for_approved_targets_only",
    sourceFetch: true,
    ...
    inputs: {
      finalExplicitControlledSportomediaGraphqlQueryBodyRecoveryExecutionRunApproval: args.approvalInput
    },
    summary: {
      controlledSportomediaGraphqlQueryBodyRecoveryExecutionCompetitionCount: executionRows.length,
      controlledQueryBodyRecoveryExecutedCount: executionRows.filter((row) => row.fetchExecutedNow).length,
      configuredGraphqlPayloadFetchExecutedCount: executionRows.filter((row) => row.fetchExecutedNow).length,
      requestBodyBuiltCount: executionRows.filter((row) => row.requestBodyBuildStatus !== "blocked_unable_to_build_request_body_from_primary_candidate").length,
      requestBodyBuildBlockedCount: executionRows.filter((row) => row.requestBodyBuildStatus === "blocked_unable_to_build_request_body_from_primary_candidate").length,
      ...
      queryBodyRecoveryCandidateResponseCount: executionRows.filter((row) => row.queryBodyRecoveryResponseCandidate).length,
      ...
      queryBodyCandidateResponseTruthCount: 0,
      canonicalWrites: 0,
      productionWrite: false,
      userHintUsedCount: 0,
      ...
      recommendedNextLane:
        executionRows.some((row) => row.queryBodyRecoveryResponseCandidate)
          ? "review_query_body_recovery_candidate_response_and_build_no_write_sportomedia_payload_parser"
          : "review_query_body_recovery_response_and_refine_request_body_candidate"
    },
    counts: {
    ...
    guardrails: [
      "This runner executes only the final-approved Sportomedia GraphQL query/body recovery targets for swe.1 and swe.2.",
      "It may fetch only the configured Sportomedia GraphQL endpoint for approved query/body recovery targets.",
      "It does not search.",
      ...
      "Query/body recovery responses are diagnostic candidates only.",
  ...
  console.log(JSON.stringify({
    output: args.output,
    controlledSportomediaGraphqlQueryBodyRecoveryExecutionCompetitionCount: output.summary.controlledSportomediaGraphqlQueryBodyRecoveryExecutionCompetitionCount,
    controlledQueryBodyRecoveryExecutedCount: output.summary.controlledQueryBodyRecoveryExecutedCount,
    ...
    queryBodyRecoveryCandidateResponseCount: output.summary.queryBodyRecoveryCandidateResponseCount,
    ...
    queryBodyCandidateResponseTruthCount: output.summary.queryBodyCandidateResponseTruthCount,
