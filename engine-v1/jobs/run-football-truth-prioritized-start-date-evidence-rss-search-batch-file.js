...
const queriesPerTarget = Number(argv[argv.indexOf("--queries-per-target") + 1] || 3);
...
const OUT_DIR = path.join(DATA_ROOT, "_diagnostics", `prioritized-start-date-evidence-rss-search-${DATE}`);
...
function isDateContext(text) {
  return /\b(start|starts|begin|begins|opening|kick.?off|fixture|fixtures|schedule|calendar|matchday|round 1|jornada|spieltag|calendario|calendrier|programma|season|2026.?27|2026.?2027)\b/i.test(String(text || ""));
}
...
async function searchBingRss(query) {
  ...
  try {
    ...
    const res = await fetch(url, {
      ...
      headers: {
        "user-agent": "Mozilla/5.0 FootballTruthStartDateEvidenceRSS/1.0",
...
function classifyTarget(target, resultRows) {
  ...
  const best = officialHostDateCandidates[0] || dateCandidates[0] || officialHostResults[0] || resultRows[0] || null;
...
const summary = {
  status: "passed",
  runner: "prioritized_start_date_evidence_rss_search_batch",
  ...
  recommendedNextLane: classifications.some((c) => c.dateCandidateCount > 0)
    ? "fetch_and_verify_candidate_pages_for_date_candidates_only"
    : "switch_to_repo_existing_search_provider_or_browser_search_because_search_endpoint_returned_no_candidates"
};

const reportPath = path.join(OUT_DIR, `prioritized-start-date-evidence-rss-search-report-${DATE}.json`);
const rowsPath = path.join(OUT_DIR, `prioritized-start-date-evidence-rss-search-results-${DATE}.jsonl`);
const classPath = path.join(OUT_DIR, `prioritized-start-date-evidence-rss-classifications-${DATE}.jsonl`);
const failuresPath = path.join(OUT_DIR, `prioritized-start-date-evidence-rss-search-failures-${DATE}.jsonl`);
const healthPath = path.join(OUT_DIR, `prioritized-start-date-evidence-rss-search-health-${DATE}.jsonl`);
