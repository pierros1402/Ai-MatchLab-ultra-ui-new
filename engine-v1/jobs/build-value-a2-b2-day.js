...
export function shouldFreezeAdjustedValueObservations(
  ...
  return Boolean(
    isDayKey(dayKey) &&
    isDayKey(calendarDay) &&
    String(dayKey) <= String(calendarDay)
  );
}

function normalizedFrozenA2(dayKey, plan, audit) {
...
function normalizedFrozenB2(dayKey, plan, audit) {
  const picks = Array.isArray(plan?.picks) ? plan.picks : null;
  const count = Number(plan?.count);
  const fixtureUniverse = plan?.sourceContract?.fixtureUniverse || audit?.membership?.fixtureUniverse || null;
...
export function readFrozenAdjustedValueObservations(dayKey) {
  ...
  return {
    A2: normalizedFrozenA2(dayKey, planA2, auditA2),
    B2: normalizedFrozenB2(dayKey, planB2, auditB2)
  };
}
