...
export function verifySnapshotCoverageFloor(options = {}) {
  ...
  if (fixtureCount < minFixtures) {
    failures.push(`fixture_count_below_floor:${fixtureCount}<${minFixtures}`);
  }
  if (leagueCount < minLeagues) {
    failures.push(`league_count_below_floor:${leagueCount}<${minLeagues}`);
  }
